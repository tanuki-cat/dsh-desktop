...
- **绘制看护**：Harness 窗口每 15s 被问一次"你还在画吗"（`eval_with_callback`，5s 超时）。页面里的探测脚本维护**两个**计数器并一起报回来 —— **动画帧**与 **`setTimeout(0)` 心跳**：帧不动但心跳在跳 = 任务队列还活着、只是不被绘制（WebKit 对判为无人观看的窗口就这样），页面完好，**不重载**，只在标题与日志里说明；帧和心跳都不动 = 主线程忙或挂了；**完全没应答 = 渲染进程没了**。后两种连续 3 次才重载当前 URL（最多 3 次）。聚焦窗口会让看护立刻判定一次，而不是等下一个周期。macOS 上另有 `on_web_content_process_terminate` 回调，渲染进程一结束就立刻重载。加载完成标题自动恢复；3 次仍不回来则停在"页面已停止刷新，请重启应用"并记日志
...
- 离线测试：`cargo test` **188 passed**，另有 **7 个集成用例**（在真实 node 引擎里跑兼容层与探测脚本）。单测覆盖：URL 解析含 LAN 后缀、凭据脱敏（`Bearer`/`api_key`/`Cookie` 等）、状态文件往返、locator 软链解析与
  ...
  **标签选择**（alpha 可以是最新版本、alpha 落后时 latest 仍胜出、registry 没有该标签时只要另一个匹配就不算失败）、
  ...
  聚焦后仍不绘制会并入重载判定、探测脚本保持 ES5 且帧与心跳各自一次只排一个回调、
...
的两条路径 —— ① 手动重载（菜单 View → 重新加载界面 / ⌘R）真的重新加载当前会话；② 把 harness 窗口切到后台
再切回来，日志出现 `页面停止绘制：计时器仍在运行`、标题出现"已暂停绘制"，且**没有**触发重载
（旧行为是每个周期记一条"窗口不在前台…本轮不判定"并在极端情况下重载）。
...
- **页面看着没死但模型输出不刷新**：这是**绘制**停了，不是连接断了 —— Harness UI 把流式输出合并到动画帧上（`requestAnimationFrame`），而输入框走的是同步刷新，所以"能打字、能发送、输出不动"完全可能同时成立。看护用两个计数器把两种停画分开（见行为表）：**帧停了但计时器还在跑** = 被 WebKit 暂停绘制，页面完好、**不重载**，标题会提示"已暂停绘制：聚焦窗口或重新加载界面即可恢复"；**两者都停** = 主线程忙或挂了，才会重载。日志关键字 `Harness 页面停止绘制`。注意 `background_throttling(Disabled)` 并**不足以**阻止停画：实测 0.4.4 上窗口失去焦点时帧计数仍连续数千次不动（2026-09-18 现场日志），所以恢复手段是"回到窗口"与 ⌘R（`重新加载界面`），而不是等看护重载。渲染进程被系统结束时另有一条立即重载的路径（日志 `WebView 渲染进程已结束`；崩溃报告在 `~/Library/Logs/DiagnosticReports`）
...
| 页面停止绘制（能点击能输入、模型输出不更新） | 15 s 一次的探测同时读**动画帧计数**与**计时器计数**。**帧不动、计时器在动** = 被 WebKit 暂停绘制（多为窗口失去焦点），页面本身完好：**不重载**，标题改为"已暂停绘制：聚焦窗口或重新加载界面即可恢复"，日志记 `页面停止绘制：计时器仍在运行`（每个连续段只记一条）。聚焦窗口会立即触发一次判定，不必等下一个周期；若聚焦后仍不绘制，它按"未响应"并入下面那条。**帧和计时器都不动**（主线程忙/挂）或**探测无应答**：连续 3 次未通过才重载当前 URL（最多 3 次），标题显示"正在重新加载…"。**重载前会先问页面是否正在被输入**：焦点在输入框、或最近 15 s 内有按键/粘贴 → 推迟一轮（标题显示"等待输入结束…"）；输入停下后照常重载，重载预算不受影响 |
...
| 手动恢复界面 | 菜单 **View → 重新加载界面**（⌘R）：重新加载 Harness 当前 URL。与看护走同一条路径，丢掉页面内存里的东西（滚动位置、展开的 Think 行、未发出的草稿），但**不丢会话**——会话在 Harness 进程里。这是"页面只是被暂停绘制"时唯一需要的手段，也是不必退出应用就能拿到新文档的入口 |
