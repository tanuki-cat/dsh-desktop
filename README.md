...
- 离线测试：`cargo test` **222 passed**，另有 **7 个集成用例**（在真实 node 引擎里跑兼容层与探测脚本）。单测覆盖：URL 解析含 LAN 后缀、凭据脱敏（`Bearer`/`api_key`/`Cookie` 等）、状态文件往返、locator 软链解析与
  ...
  **插件市场重启的替代实例按命令行重放识别**（本壳 overlay 才算自家、终端 `dsh web` 与别人的 overlay 都不算、
  只提到路径的无关程序不因"像 dsh"被误判）、**恢复面判定**（市场载荷的 `"recovery":true` 才算、
  宿主自己的 status 载荷不算、点名插件与没点名分开、释放请求要有应答才算成功 —— 都在真实 socket 上跑）、
  **重试判据的子进程存活测试**（`try_wait` 的三条答复：已退出、已被外部 reap 掉（`ECHILD`）、仍在运行；
  这条用例经过变异验证 —— 把 `ECHILD` 读成"还在跑"，它会失败）、
  未读完的输出丢掉残行、
...
| 端口上是**本应用自己那条命令行**的重放（插件市场的「立即重启」把宿主 SIGTERM 后，detached helper 按原 argv 拉起的替代实例） | **不弹面板、直接按自家实例处理**：判据是命令行同时像 `dsh web` **且**带本壳独有的 `--patch <app-data>/force-print-url.yml`（只有本壳会传这个 overlay；终端 `dsh web` 没有，官方桌面端用自己的）。这类实例**既不能复用也不是外人**：它重放的 argv 连 cwd 都是 CLI 目录而不是本壳配置的 workspace，所以只能停掉重起；而弹面板问用户"要不要接管"，唯一走得通的答复恰好就是接管 —— 那一问一答本身会把市场正在等的替代实例杀掉，市场于是判定自己重启失败、把恢复页放到端口上，本壳随后的 spawn 撞 `EADDRINUSE`（2026-09-24 实测，市场 helper 日志与本壳日志逐条对得上） |
...
| 端口上是插件市场的**恢复页**（上次插件更新后 Harness 没起来，市场把端口接管了） | 不当作"别的程序"：先 `GET /dsh-market/recovery` 读它的载荷。**点名了插件**（`implicated`）→ 那一页的勾选清单是关掉报错插件的唯一入口，**不抢端口**，用系统浏览器打开它并说明"改完回来点重新启动"；**没点名插件**（页面只是占着端口，默认要占 15 分钟）→ `POST /dsh-market/recovery/release` 请它释放，等端口真空出来后继续启动（最多 3 s）。两处判据都来自市场自己的载荷：恢复面独有的 `"recovery":true`，而**宿主自己的** `/dsh-market/status` 没有这个字段 |
| 恢复页在**检测之后、spawn 期间**才抢到端口（本次事故走的就是这条） | `wait_for_url` 失败时若子进程**已经退出**（`EADDRINUSE` 的形状）且端口上是没点名插件的恢复页 → 请求释放后**重跑一次 spawn**（仅一次）。检测那一步看不到这个窗口：市场 helper 要先等满 8 s 才判定替代实例失败并起恢复面，壳的 spawn 正落在那 8 s 里。判"已经退出"用的是 `try_wait`（`child_is_gone`）而不是 `process::is_alive`：后者是 `kill(pid, 0)`，对**已退出但未被 reap** 的子进程仍答"活着"，用它就永远不会重试 |
...
| 启动成功后 Harness 意外退出 | 看护线程发现退出 → 清 state.json → 销毁 Harness 窗口、状态页显示"正在重新启动…" → **自动重启**：干净退出（退出码 0，插件市场「立即重启」就是这一种）先等 8s，端口被替代实例接管时先停掉它再按本壳的 workspace 启自己的（替代实例按上面那条"命令行重放"识别，**不问用户**）；崩溃/被杀 2s 后直接自启。成功后切到新的 token URL；连续 3 次短命重启未稳定才停在终态报错页（带「重新启动 Harness」按钮） |
