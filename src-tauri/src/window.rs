...
pub struct WebviewReport {
    ...
    /// Syntax this engine cannot parse: the splash page probes for it (`src/index.html`).
    #[serde(default)]
...
/// its IPC bridge in its own script, and the syntax verdict comes from the page's own head. The
/// report therefore waits for both and then gives up quietly — a diagnostic that failed must not
/// decide anything.
