...
- `stop_instance_before_update()` 仍在最前：树被换走时运行中的 harness 下一次 lazy require 会
  ...
  **2026-09-16 修正**：它的前置判据 `may_stop_before_update()` 不再接收 `take_over_existing` —— 该函数
  ...
  > **2026-09-17 补正（`docs/dsh-desktop-v0.4.4-code-review.md` 的 A1）**：上面第一条「`stop_instance_before_update()`
  > 仍在最前」在 `64a2041` 之后**已不成立** —— 该调用被从核心更新路径移除，停止改由 3c 的检测分支负责，
  > 而 3c 只识别 state.json 命中的实例。故障路径（提交后启动失败 → 回滚 → 自动重启）里，未被登记的旧实例
  > 会继续服务被 rename 的树，重启因此必然失败。补的守卫是 `lib.rs::swap_blocked()` 的第三个输入
  > `raced_original`：原端口上仍有 Harness 时放弃本次切换。本条保留原文，仅作交叉引用。

## 3. 验证

### 3.1 单测（库内 116 → 142，本项占 14 项）
...
`cargo test` **142 passed / 0 failed**（另有 6 项集成用例）、`cargo fmt --check` 通过、`cargo clippy --all-targets`
0 warning。
