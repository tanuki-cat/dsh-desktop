...
**本轮已修 7 项**（1、2、5、8、9、11、14），**部分完成 3 项**（6、7、13），**未做 5 项**（3、4、10、12、15）。
...
- 第 5 条建议：**未做**。改为「默认不接管」消解了误杀风险，但没有实现交互确认。仓库当前没有任何对话框
  依赖（`Cargo.toml` 只有 `tauri` 与 `single-instance`），实现它需要引入 `tauri-plugin-dialog` 并把接管
  路径改成异步确认。用户显式设 `take_over_existing: true` 时，接管仍是静默发生的。
...
本轮未实现，作为独立特性单列。这是本轮唯一「有明确方案但工作量成规模」的项：临时目录安装、安装结果
完整性校验、原子目录切换、last-known-good 与失败回退，需要把 `start()` 的「解析 → 起进程 → 等 URL」
包成可重试的一轮，并引入目录级版本仲裁。README 与方案文档均已标注未做。
...
**未做**：真正的交互式选择（弹窗让用户当场选）。当前是「默认安全 + 配置项切换」，不是运行时询问；
原因见第 1 项第 5 条建议（需要新增对话框依赖）。
...
- 其中「更新中断后的安装目录恢复」依赖第 10 项（未做），当前没有可恢复机制，该验证项在本轮之后仍无
  对应实现。
...
1. **接管前的交互确认**（第 1 项第 5 条建议、第 14 项）—— 当前是「默认不接管 + 配置项切换」，
   不是运行时询问；需要引入对话框依赖。这是本轮唯一还带安全语义的缺口。
2. **更新事务与回滚**（第 10 项）—— 本轮唯一有明确方案但未动工的成规模特性。
