...
## 11. 后续复核补充（2026-09-17）

第四轮审查（[`dsh-desktop-v0.4.4-code-review.md`](./dsh-desktop-v0.4.4-code-review.md)）复核了本文 §9 表格里的修复，
发现其中**第 23 项失实**：`Iterator#reduce` 下标那条修复的回归测试注入的是空脚本
（`compat_script(&["Iterator.prototype.reduce"])` 与块名 `"Iterator"` 全等不匹配），断言跑在 node 原生实现上，
因此该修复当时没有被任何测试覆盖。测试已在第四轮改写为真正剥离原实现后再注入，并用变异测试验证可捕获回归。
本文其余各项结论维持不变。
